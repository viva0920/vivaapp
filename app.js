...
(function () {
  ...
  (function initImport() {
    ...
    const parseBtn = $("#importParse");
    const resultBox = $("#importResult");
    const nameInput = $("#importName");
    const ingInput = $("#importIng");
    const stepsInput = $("#importSteps");
    const saveBtn = $("#importSave");
    const cancelBtn = $("#importCancel");

    function resetModal() {
      textArea.value = ""; resultBox.hidden = true;
      parseBtn.disabled = true; saveBtn.disabled = true;
      nameInput.value = ""; ingInput.value = ""; stepsInput.value = "";
    }
    function openModal() { resetModal(); modal.classList.add("show"); setTimeout(() => textArea.focus(), 50); }
    ...
    function pickEmoji(name) {
      ...
      for (const [re, em] of map) if (re.test(name)) return em;
      return "🧁";
    }
    ...
    function parseXcf(raw) {
      ...
      const steps = stepsRaw
        .map((x) => x.replace(/^(?:步骤|做法)?\s*\d+[\.、)）:]?\s*/, "").trim())
        .filter(Boolean);
      return {
  ...
  function stickerSvg(kind) {
    if (kind === "cat") return '<img class="sticker-img" src="sticker-cat.png?v=17" alt="小猫" />';
    if (kind === "dog") return '<img class="sticker-img" src="sticker-dog.png?v=17" alt="小狗" />';
    if (kind === "rabbit") return '<img class="sticker-img" src="sticker-rabbit.png?v=17" alt="小兔" />';
    if (kind === "deer") return '<img class="sticker-img" src="sticker-deer.png?v=17" alt="小鹿" />';
    if (kind === "hedgehog") return '<img class="sticker-img" src="sticker-hedgehog.png?v=17" alt="刺猬" />';
    return "";
  }
  ...
  function buildCoverCalendar(ym) {
    ...
    for (let i = 0; i < 42; i++) {
      ...
      if (dayNum < 1 || dayNum > daysInMonth) { cells += '<div class="monthbook-cell empty"></div>'; continue; }
      ...
      let dots = "";
      if (imgCount) dots = '<div class="cell-dots">' + Array(imgCount).fill('<span></span>').join("") + "</div>";
      let inner = "";
      if (isToday) inner += '<div class="cell-today-circle">' + (me ? me : '<span class="num">' + dayNum + "</span>") + "</div>";
      else inner += '<span class="monthbook-cell-day">' + dayNum + "</span>";
      if (has) inner += (me && !isToday ? '<div class="cell-mood">' + me + "</div>" : "") + dots + sticker + more;
      cells += '<div class="monthbook-cell' + (has ? " has" : "") + (isToday ? " today" : "") + '" data-date="' + dateStr + '">' + inner + "</div>";
    }
